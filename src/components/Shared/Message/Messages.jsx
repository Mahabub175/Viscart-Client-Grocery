...
const socket = io("https://api.komedei.com");
...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    const handleNewUserMessage = (newMessageData) => {
      if (
        newMessageData.conversationId === deviceId ||
        newMessageData.userId === user?._id
      ) {
        setLocalUserMessages((prevMessages) => [
          ...prevMessages,
    ...
    const handleNewAdminMessage = (newMessageData) => {
      if (
        newMessageData.conversationId === deviceId ||
        newMessageData.userId === user?._id
      ) {
    ...
    socket.on("newUser Message", handleNewUserMessage);
    socket.on("newAdminMessage", handleNewAdminMessage);

    return () => {
      socket.off("newUser Message", handleNewUserMessage);
